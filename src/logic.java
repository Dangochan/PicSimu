...
public class logic {
	...
	void commandBCF() {
		...
		sto.writeStorage(checkZeroF(extractF()),
				(sto.getDataStorage(checkZeroF(extractF())) & mask));
		sto.setZ(tempZ);
		// Standardanweisungen
		increasePC();
		sto.incTime();
	...
	void commandDECF() {
		...
		if (extractD() == 0) {
			...
			sto.writeStorage(checkZeroF(extractF()),
					sto.getDataStorage(checkZeroF(extractF())) - 1);
		}
		// Standardanweisungen
		increasePC();
		sto.incTime();
	...
	void commandRRF() {
		...
		System.out.println("der Wert an der Speicherstelle ist "
				+ sto.getDataStorage(checkZeroF(extractF())));
		System.out.println("Das 0bit ist "
				+ (sto.getDataStorage(checkZeroF(extractF())) & 0B1));
	...
	void commandSUBWF() {
		...
		int ergebnis = sto.getDataStorage(checkZeroF(extractF()))
				+ subtract(sto.getW());
		sto.testAndSetDC(sto.getDataStorage(checkZeroF(extractF())),
				subtract(sto.getW()));
		sto.testAndSetC(sto.getDataStorage(checkZeroF(extractF())),
				subtract(sto.getW()));
		if (extractD() == 0) {
			sto.setW(ergebnis);
		} else {
	...
	void commandSWAPF() {
		boolean tempZ = sto.getZ();
		int ergebnis = (((sto.getDataStorage(checkZeroF(extractF())) & 0B1111) << 4) | ((sto
				.getDataStorage(checkZeroF(extractF())) & 0B11110000) >> 4));
		if (extractD() == 0) {
			sto.setW(ergebnis);
		} else {
			sto.writeStorage(checkZeroF(extractF()), ergebnis);
